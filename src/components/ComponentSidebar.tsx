...
export const ComponentSidebar = ({
  ...
  useEffect(() => {
    // Scroll active button into view in the sidebar
      ...
      <div className="lg:hidden fixed top-[72px] left-0 right-0 z-40 bg-white dark:bg-gray-900 p-4 shadow-md">
        ...
        >
          <span className="text-gray-700 dark:text-gray-200">
            {components.find(
              (c) => c.title.toLowerCase() === activeSection.toLowerCase()
      ...
      >
        <nav className="space-y-1 py-4">
          {components.map((component) => (
            <motion.button
              key={component.title}
              ref={component.title === activeSection ? activeButtonRef : null}
              onClick={() => scrollToSection(component.title)}
